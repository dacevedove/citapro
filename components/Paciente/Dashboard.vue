    ...
    methods: {
      async cargarCitas() {
        ...
        try {
          ...
          if (this.activeTab === 'pending') {
            const response = await axios.get('https://localhost/lgm/api/citas/listar-paciente.php?tipo=pendientes', {
              headers: { 'Authorization': `Bearer ${token}` }
            });
            
            this.citasPendientes = response.data;
          } else {
            const response = await axios.get('https://localhost/lgm/api/citas/listar-paciente.php?tipo=historial', {
              headers: { 'Authorization': `Bearer ${token}` }
            });
      ...
      async cancelarCita() {
        ...
        try {
          ...
          const response = await axios.put('https://localhost/lgm/api/citas/cancelar-paciente.php', {

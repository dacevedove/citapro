    ...
    methods: {
      async cargarDatos() {
        ...
        try {
          ...
          const espResponse = await axios.get('/api/doctores/especialidades.php', {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          this.especialidades = espResponse.data;
          
          // Cargar doctores
          const docResponse = await axios.get('/api/doctores/listar.php', {
            headers: { 'Authorization': `Bearer ${token}` }
          });
          this.doctores = docResponse.data;
          
          // Cargar aseguradoras
          const asegResponse = await axios.get('/api/aseguradoras/listar.php', {
            headers: { 'Authorization': `Bearer ${token}` }
          });

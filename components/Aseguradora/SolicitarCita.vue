  ...
  methods: {
    async cargarEspecialidades() {
      try {
        ...
        const response = await axios.get('https://localhost/lgm/api/doctores/especialidades.php', {
    ...
    async buscarTitular() {
      ...
      try {
        this.titular = null;
        const token = localStorage.getItem('token');
        const response = await axios.get(`https://localhost/lgm/api/titulares/buscar.php?cedula=${this.cedulaTitular}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
    ...
    async buscarPaciente() {
      ...
      try {
        this.paciente = null;
        const token = localStorage.getItem('token');
        const response = await axios.get(`https://localhost/lgm/api/pacientes/buscar.php?cedula=${this.cedulaPaciente}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
    ...
    async enviarSolicitud() {
      ...
      try {
        ...
        if (!titularId) {
          // Crear nuevo titular
          const titularResponse = await axios.post('https://localhost/lgm/api/titulares/crear.php', 
            {
              ...this.nuevoTitular,
        ...
        if (!pacienteId) {
          if (this.esTitularPaciente) {
            ...
            const pacienteResponse = await axios.get(`https://localhost/lgm/api/pacientes/buscar.php?titular_id=${titularId}&es_titular=1`, {
              headers: { 'Authorization': `Bearer ${token}` }
            });
            ...
            const pacienteResponse = await axios.post('https://localhost/lgm/api/pacientes/crear.php',
              {
                ...this.nuevoPaciente,
        ...
        // 3. Crear la cita
        const citaResponse = await axios.post('https://localhost/lgm/api/citas/crear.php',
          {
